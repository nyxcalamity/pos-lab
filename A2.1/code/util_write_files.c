...
//	printf("%s\n", file_out_name);
	...
	sprintf( szFileName, "%s%s.cgup.cutted.rank%i.vtk",
			"./out/",file_out_name, myrank);
	vtk_for_process(file_in, szFileName,
			0, local_num_elems-1, points_count, points, elems,
			local_global_index, local_num_elems, scalars);
	// Output SU
	for(i=0;i<local_num_elems;i++){
		scalars[i] = su[i];
	}
	sprintf( szFileName, "%s%s.su.rank%i.vtk",
			"./out/",file_out_name, myrank);
	test_distribution(file_in, szFileName,
			local_global_index, local_num_elems, scalars);
	sprintf( szFileName, "%s%s.su.cutted.rank%i.vtk",
			"./out/",file_out_name, myrank);
	vtk_for_process(file_in, szFileName,
			0, local_num_elems-1, points_count, points, elems,
			local_global_index, local_num_elems, scalars);
