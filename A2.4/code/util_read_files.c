...
int read_geometry(char *file_name, int *NINTCI, int *NINTCF, int *NEXTCI, int *NEXTCF,
    ...
    fseek(fp, (4+6*(*NINTCF+1))*sizeof(int) + 8*(*NINTCF+1)*sizeof(double), SEEK_SET);
...
int read_lcc_local(char *file_name, int NINTCI, int NINTCF, int **LCC, int *local_global_index) {
    ...
    for ( i = NINTCI; i <= NINTCF; i++ ) {
        fseek(fp, (4+6*local_global_index[i])*sizeof(int), SEEK_SET);
...
int read_boundary_local(char *file_name, int NINTCI, int NINTCF, int nintcf_g,
    ...
    for ( i = NINTCI; i <= NINTCF; i++ ) {
        fseek(fp, (4+6*(nintcf_g+1))*sizeof(int)+(8*local_global_index[i])*sizeof(double), SEEK_SET);
        fread(&(BS[i]), sizeof(double), 1, fp);
        fread(&(BE[i]), sizeof(double), 1, fp);
        fread(&(BN[i]), sizeof(double), 1, fp);
        fread(&(BW[i]), sizeof(double), 1, fp);
